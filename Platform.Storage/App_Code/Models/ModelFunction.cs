...
public static class ModelFunction
{
                ...
                case "RadNumericTextBox":
                    {
                        ...
                        {
                            (controls[i] as RadNumericTextBox).Value = (double?)values[i];
                        }
                        break;
                    }
                case "RadComboBox":
    ...
    public static Guid StorageObjectAdd(this StorageEntity db, Guid storageId, Guid catalogId, string name, string unit, string specification, bool consumable, bool @single, bool @fixed, string fixedSerial, decimal low, decimal high, Models.Image image, Guid @operator, int ordinal, DateTime time, string note, string code)
    {
    ...
    public static void StorageObjectEdit(this StorageEntity db, Guid storageId, Guid id, string name, string unit, string specification, string fixedSerial, decimal low, decimal high, Models.Image image, int ordinal, string note, string code)
    {
    ...
    public static Guid SetFlow(this StorageEntity db, Guid storageId, StorageObject obj, FlowType type, DateTime time, int timeNode, decimal amount, decimal perPrice, decimal additionalFee, Guid? relativeId, Guid? relativeSingleId, string note)
    {
    ...
    public static Guid SetFlow(this StorageEntity db, Guid storageId, StorageObject obj, FlowType type, DateTime time, int timeNode, decimal total, string note, decimal @in, decimal lend, decimal @out)
    {
        StorageFlow last = obj.LastFlowId == Guid.Empty ? null : db.StorageFlow.SingleOrDefault(o => o.Id == obj.LastFlowId);
        var flow = new StorageFlow();
        flow.Id = db.GlobalId();
        ...
        flow.Amount = 0;
        flow.PerPrice = 0.00M;
        flow.TotalPrice = 0.00M;
        flow.AdditionalFee = total;
        flow.RelativeId = Guid.Empty;
        flow.RelativeSingleId = Guid.Empty;
        flow.InitialInAmount = last == null ? 0 : last.FinalInAmount;
        flow.InitialInMoney = last == null ? 0.00M : last.FinalInMoney;
        flow.InitialLendAmount = last == null ? 0 : last.FinalLendAmount;
        flow.InitialLendMoney = last == null ? 0.00M : last.FinalLendMoney;
        ...
        flow.InitialTotalAmount = last == null ? 0 : last.FinalTotalAmount;
        flow.InitialTotalMoney = last == null ? 0.00M : last.FinalTotalMoney;
        flow.FinalInAmount = flow.InitialInAmount;
        flow.FinalInMoney = flow.InitialInMoney + @in;
        flow.FinalLendAmount = flow.InitialLendAmount;
        flow.FinalLendMoney = flow.InitialLendMoney + lend;
        flow.FinalOutAmount = flow.InitialOutAmount;
        flow.FinalOutMoney = flow.InitialOutMoney + @out;
        flow.FinalTotalAmount = flow.FinalInAmount + flow.FinalLendAmount + flow.FinalOutAmount;
        flow.FinalTotalMoney = flow.FinalInMoney + flow.FinalLendMoney + flow.FinalOutMoney;
        flow.Note = note;
        db.StorageFlow.Add(flow);
    ...
    public static Guid SetFlow(this StorageEntity db, Guid storageId, StorageObject obj, FlowType type, DateTime time, int timeNode, decimal total, string note, decimal amount)
    {
        StorageFlow last = obj.LastFlowId == Guid.Empty ? null : db.StorageFlow.SingleOrDefault(o => o.Id == obj.LastFlowId);
        var flow = new StorageFlow();
        flow.Id = db.GlobalId();
        ...
        flow.InitialTotalAmount = last == null ? 0 : last.FinalTotalAmount;
        flow.InitialTotalMoney = last == null ? 0.00M : last.FinalTotalMoney;
        flow.FinalInAmount = flow.InitialInAmount + amount;
        flow.FinalInMoney = flow.InitialInMoney + total;
        flow.FinalLendAmount = flow.InitialLendAmount;
        flow.FinalLendMoney = flow.InitialLendMoney;
        flow.FinalOutAmount = flow.InitialOutAmount;
        flow.FinalOutMoney = flow.InitialOutMoney;
        flow.FinalTotalAmount = flow.FinalInAmount + flow.FinalLendAmount + flow.FinalOutAmount;
        flow.FinalTotalMoney = flow.FinalInMoney + flow.FinalLendMoney + flow.FinalOutMoney;
        flow.Note = note;
        db.StorageFlow.Add(flow);
    ...
    public static Guid SetIn(this StorageEntity db, Guid objectId, Guid targetId, string age, string place, Guid? responsible, Guid @operator, decimal amount, decimal price, decimal fee, string note)
    {
    ...
    public static Guid SetConsume(this StorageEntity db, Guid objectId, Guid? consumer, Guid @operator, decimal amount, string note)
    {
    ...
    public static bool SetOutM(this StorageEntity db, Guid objectId, Guid? outUser, Guid @operator, OutType type, string reason, decimal amount, string note)
    {
        try
        {
    ...
    public static Guid SetOutMDo(this StorageEntity db, Guid objectId, Guid? outUser, Guid @operator, OutType type, string reason, decimal amount, string note)
    {
        var time = DateTime.Now;
        var timeNode = int.Parse(time.ToString("yyyyMMdd"));
        var obj = db.StorageObject.Single(o => o.Id == objectId);
    ...
    public static Guid SetLendM(this StorageEntity db, Guid objectId, Guid? outUser, Guid @operator, decimal amount, string note)
    {
        var time = DateTime.Now;
        var timeNode = int.Parse(time.ToString("yyyyMMdd"));
        var obj = db.StorageObject.Single(o => o.Id == objectId);
    ...
    public static Guid SetReturnM(this StorageEntity db, Guid objectId, Guid borrowUser, Guid @operator, decimal amount, string note)
    {
