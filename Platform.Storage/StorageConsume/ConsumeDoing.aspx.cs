...
public partial class ConsumeDoing : SingleStoragePage
{
    ...
    protected bool InRecord(out Guid cid)
    {
        ...
        cid = db.Value.SetConsume(ObjectId, responsibleId.Value.Null() ? (Guid?)null : responsibleId.Value.GlobalId(), CurrentUser, in_amount.Value(0.00M), in_note.Text);
    ...
    protected void in_Click(object sender, System.Web.UI.ImageClickEventArgs e)
    {
        ...
        var uid = responsibleId.Value.Null() ? CurrentUser : responsibleId.Value.GlobalId();
        Response.Redirect("~/StorageConsume/ConsumeDone?StorageId={0}&ObjectId={1}&ConsumeId={2}&UserId={3}".Formatted(StorageId, ObjectId, cid, uid));
    }
}
