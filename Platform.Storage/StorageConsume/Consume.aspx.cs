...
public partial class Consume : SingleStoragePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        obj_search.DataSource = db.Value.QueryStorageObject("", StorageId).Where(o => o.Consumable == true).ToList();
    }

    protected void obj_search_DataSourceSelect(object sender, Telerik.Web.UI.SearchBoxDataSourceSelectEventArgs e)
    {
        obj_search.DataSource = db.Value.QueryStorageObject(e.FilterString, StorageId).Where(o => o.Consumable == true);
        obj_view.Rebind();
    }
    ...
    {
        Response.Redirect("~/StorageConsume/ConsumeDoing?StorageId={0}&ObjectId={1}".Formatted(StorageId, obj_id.Value));
    }
