...
public partial class LendDoneS : SingleStoragePage
{
    ...
    {
        Response.Redirect("~/StorageLend/LendDoingS?StorageId={0}&ObjectId={1}".Formatted(StorageId, ObjectId));
    }
    ...
    protected void view_NeedDataSource(object sender, RadListViewNeedDataSourceEventArgs e)
    {
        var outId = LendId;
        var @out = db.Value.StorageLend.Single(o => o.Id == outId);
        view.Source(@out.StorageLendSingle.Select(o => o.Ordinal).ToList());
    }
}
