...
public partial class OutDoneS : SingleStoragePage
{
    ...
    {
        Response.Redirect("~/StorageOut/OutDoingS?StorageId={0}&ObjectId={1}".Formatted(StorageId, ObjectId));
    }
    ...
    protected void view_NeedDataSource(object sender, RadListViewNeedDataSourceEventArgs e)
    {
        var outId = OutId;
        var @out = db.Value.StorageOut.Single(o => o.Id == outId);
        view.Source(@out.StorageOutSingle.Select(o => o.Ordinal).ToList());
    }
}
