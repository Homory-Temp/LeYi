...
public partial class QueryIn : SingleStoragePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            InitQueryIn();
            view.MasterTableView.Columns[13].Visible = Right_Create;
        }
    }
    ...
    protected void view_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
    {
        ...
        var source = tree.CheckedNodes.Select(o => o.Value.GlobalId()).ToList().Join(db.Value.查询_入库单, o => o, o => o.分类标识, (x, y) => y).ToList().Where(o => (o.物品名称.ToLower().Contains(n) || o.拼音.ToLower().Contains(n)) && o.日期 > ds && o.日期 < de && (o.责任人.ToLower().Contains(u))).OrderByDescending(o => o.时间).ToList();
    ...
    {
        return db.Value.查询_入库单.Count(o => o.物品标识 == objectId);
    }
