...
public partial class QueryConsume : SingleStoragePage
{
    ...
    protected void view_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
    {
        ...
        var source = tree.CheckedNodes.Select(o => o.Value.GlobalId()).ToList().Join(db.Value.查询_领用单, o => o, o => o.分类标识, (x, y) => y).ToList().Where(o => (o.物品名称.ToLower().Contains(n) || o.拼音.ToLower().Contains(n)) && o.日期 > ds && o.日期 < de && (o.领用人.ToLower().Contains(u))).OrderByDescending(o => o.时间).ToList();
    ...
    {
        return db.Value.查询_领用单.Count(o => o.物品标识 == objectId);
    }
