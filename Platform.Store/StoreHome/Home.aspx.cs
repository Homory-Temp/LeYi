...
public partial class StoreHome_Home : SingleStorePage
{
    ...
    protected void view_query_NeedDataSource(object sender, Telerik.Web.UI.RadListViewNeedDataSourceEventArgs e)
    {
        ...
        else
        {
            ...
            list.Add(new HomeItem { Name = "汇总统计", Url = "../StoreQuery/StatisticsDaily?StoreId={0}".Formatted(StoreId) });
        }
        list.Add(new HomeItem { Name = "月库存查询", Url = "../StoreQuery/Statistics?StoreId={0}".Formatted(StoreId) });
        list.Add(new HomeItem { Name = "采购来源统计", Url = "../StoreQuery/StatisticsX?StoreId={0}".Formatted(StoreId) });
        if (CurrentStore.State == Models.StoreState.固产)
        {
