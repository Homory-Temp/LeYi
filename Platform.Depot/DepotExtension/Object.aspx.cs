...
public partial class DepotExtension_Object : DepotPage
{
    ...
    protected Depot Depot
    {
        get
        {
            if (_depot == null && combo.SelectedIndex >= 0)
            {
                var id = combo.SelectedValue.GlobalId();
                _depot = DataContext.Depot.SingleOrDefault(o => o.Id == id);
            }
            return _depot;
        }
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            combo.DataSource = DataContext.Depot.Where(o => o.State < State.停用).ToList().Where(o => (o.Type & DepotType.固定资产库) == DepotType.无).ToList();
            combo.DataBind();
