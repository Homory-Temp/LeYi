...
{
    public class 已阅信息Controller : OfficeController
    {
        private int per = 10;
        ...
        public ActionResult 首页()
        {
            if (string.IsNullOrEmpty(Account))
                return 认证();
            var count = DB.f______计数已阅信息数(Account).Single();
            ViewBag.Max = count % per == 0 ? count / per - 1 : (count + (per - count % per)) / per - 1;
            return View();
        }

        public ActionResult 已阅信息列表单页()
        {
            if (string.IsNullOrEmpty(Account))
                return 认证();
            var id = RouteData.Values["id"] == null ? 0 : int.Parse(RouteData.Values["id"].ToString());
            ViewBag.Current = id + 1;
            var list = DB.f______列表已阅信息表(Account).OrderByDescending(o => o.AppG_Begintime).Skip(id * per).Take(per).ToList();
            return View(list);
        }

        public ActionResult 已阅信息列表内容()
        {
            if (string.IsNullOrEmpty(Account))
                return 认证();
            var id = RouteData.Values["id"]?.ToString();
            var int_id = int.Parse(id);
            var type = DB.f______已阅信息类型表(int_id).FirstOrDefault();
            if (type.AppT_ID.StartsWith("IOA_Message", StringComparison.OrdinalIgnoreCase))
            {
                return RedirectToAction("已阅信息列表内容通知", "已阅信息", new { id = id });
            }
            else if (type.AppT_ID.StartsWith("IOA_Send", StringComparison.OrdinalIgnoreCase))
            {
                return RedirectToAction("已阅信息列表内容发文", "已阅信息", new { id = id });
            }
            else if (type.AppT_ID.StartsWith("IOA_Accept", StringComparison.OrdinalIgnoreCase))
            {
                return RedirectToAction("已阅信息列表内容收文", "已阅信息", new { id = id });
            }
            return new EmptyResult();
        }

        public ActionResult 已阅信息列表内容通知()
        {
            if (string.IsNullOrEmpty(Account))
                return 认证();
            var id = RouteData.Values["id"]?.ToString();
            var int_idx = int.Parse(id);
            var giveOut = DB.f______已阅信息类型表(int_idx).FirstOrDefault();
        ...
        public ActionResult 已阅信息列表内容发文()
        {
            if (string.IsNullOrEmpty(Account))
                return 认证();
            var id = RouteData.Values["id"]?.ToString();
            ...
            var send = DB.f______已阅信息发文表(int_id).FirstOrDefault();
        ...
        public ActionResult 已阅信息列表内容收文()
        {
            if (string.IsNullOrEmpty(Account))
                return 认证();
            var id = RouteData.Values["id"]?.ToString();
            var int_id = int.Parse(id);
            ViewBag.GiveOutId = int_id;
            var receive = DB.f______已阅信息收文表(int_id).FirstOrDefault();
