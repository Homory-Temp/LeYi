...
namespace Platform.JHMobile.Controllers
{
    public class CallController : JinHerController
    {
        ...
        public ActionResult CallRead()
        {
            ...
            var per = 10;
            if (count < id * per)
                return RedirectToAction("CallRead", "Call", new { id = id - 1 });
        ...
        public ActionResult CallHistoric()
        {
            if (string.IsNullOrEmpty(Account))
                return Authenticate();
            var count = db.f____Mobile_Count_CallHistoric(Account, null, null).Single().Value;
            var id = RouteData.Values["id"] == null ? 0 : int.Parse(RouteData.Values["id"].ToString());
            var per = 10;
            if (count < id * per)
                return RedirectToAction("CallHistoric", "Call", new { id = id - 1 });
            var list = db.f____Mobile_List_CallHistoric(Account, null, null).OrderByDescending(o => o.CallTime).Skip(id * per).Take(per).ToList();
            ViewBag.Min = 0;
            ViewBag.Max = count % per == 0 ? count / per - 1 : (count + (per - count % per)) / per - 1;
            ViewBag.Current = id;
            return View(list);
        ...
        public ActionResult CallHistoricPreview()
        {
            if (string.IsNullOrEmpty(Account))
                return Authenticate();
            var id = RouteData.Values["id"]?.ToString();
            ...
            var int_id = int.Parse(id);
            var obj = db.f____Mobile_List_CallHistoric(Account, null, null).FirstOrDefault(o => o.CallID == int_id);
            var query = db.f____Mobile_List_CallAttachment(obj.CallID.ToString()).OrderBy(o => o.SlaveID);
            var list = query == null ? new List<f____Mobile_List_CallAttachment_Result>() : query.ToList();
            foreach (var path in list)
            {
            ...
            var result = new CallHistoricObject();
            result.Object = obj;
            result.List = list;
            return View(result);
        }
